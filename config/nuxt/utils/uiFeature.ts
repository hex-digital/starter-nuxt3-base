...
try {
  uiFeaturePath = path.join(__dirname, '..', '..', '..', 'modules', 'ui')
  uiFeatureDirectories = getDirectories(uiFeaturePath).map((directory) => `~/modules/ui/${directory}/components`)
} catch (err) {
  ...
  console.warn('WARN: Check ~/modules/ui directory exists')
